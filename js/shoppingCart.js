...
// load items from sessionStorage, if  you want your data to persist outside tab or when after session loss use localStorage
    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item.sku != null && item.description != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item.sku, item.description, item.price, item.quantity);
...
shoppingCart.prototype.addItem = function (sku, descr, price, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(sku, descr, price, quantity);
            this.items.push(item);
        }
...
shoppingCart.prototype.removeItem = function (sku) {
    if (sku != null) {
        ...
        if (!found) {
            window.alert("Couldn't find item to remove.");
        }
...
shoppingCart.prototype.checkoutPayPal = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        ...
        data["item_name_" + ctr] = item.description;
...
shoppingCart.prototype.checkoutGoogle = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        ...
        data["item_description_" + ctr] = item.description;
...
function cartItem(sku, descr, price, qty) {
    this.sku = sku;
